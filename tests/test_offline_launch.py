...
class OfflineLaunch(unittest.TestCase):
    def test_saved_ip_server_off_launches_local_without_hosts_changes(self):
        ...
        with patch('iop_assets.ensure_game_files',return_value={'iop_restored':False,'graphics_installed':[]}), patch('iop_stability.apply'), patch('iop_hotkeys.is_applied',return_value=True), patch('iop_sync.fix_flame'), patch('iop_server_tab.resolve_server',return_value='26.1.2.3'), patch('iop_server_tab.socket.create_connection',side_effect=ConnectionRefusedError()), patch('iop_server_tab.diagnostics.event'):
